this.actives_tail_slam_skill <- this.inherit("scripts/skills/skill", {
	m = {
		TilesUsed = []
	},
	function create()
	{
		this.m.ID = "actives.ridable_tail_slam";
	...
	function onUse( _user, _targetTile )
	{
		...
		if (ownTile.hasNextTile(nextDir))
		{
			...
			if (success && nextTile.IsOccupiedByActor && nextTile.getEntity().isAlive() && !nextTile.getEntity().isDying() && !_user.isAlliedWith(nextTile.getEntity()))
			{
