this.actives_tail_slam_big_skill <- this.inherit("scripts/skills/skill", {
	m = {
		TilesUsed = []
	},
	function create()
	{
		this.m.ID = "actives.ridable_tail_slam_big";
	...
	function onUse( _user, _targetTile )
	{
		...
		for( local i = 5; i >= 0; i = --i )
		{
			...
			else
			{
				...
				if (tile.IsOccupiedByActor && tile.getEntity().isAttackable() && this.Math.abs(tile.Level - ownTile.Level) <= 1 && !_user.isAlliedWith(tile.getEntity()))
				{
