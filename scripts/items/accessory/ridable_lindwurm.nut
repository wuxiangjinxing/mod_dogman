this.ridable_lindwurm <- this.inherit("scripts/items/accessory/ridable_pet", {
	m = {
		Tail = null,
	},
	function create()
	{
		...
		this.m.Variant = 0;		
		this.m.ID = "accessory.ridable_lindwurm";
		this.m.Name = this.Const.Strings.WardogNames[this.Math.rand(0, this.Const.Strings.WardogNames.len() - 1)] + " the Lindwurm";		
		this.m.Description = "";
		this.m.SlotType = this.Const.ItemSlot.Accessory;
		this.m.IsDroppedAsLoot = false;
		this.m.ShowOnCharacter = false;
		this.m.Value = 1000;
		this.m.Script = "scripts/entity/tactical/enemies/riding_lindwurm";
		this.m.Type = this.Const.Tactical.Actor.Lindwurm;
		this.m.Ride = true;				
		this.m.Bodysprite = "bust_lindwurm_body_10";
		this.m.Headsprite = "bust_lindwurm_head_10";			
		this.m.Injuheadprite = "bust_lindwurm_head_100_injured";			
		this.m.Injubodysprite = "bust_lindwurm_body_100_injured";			
	...
	function onEquip()
	{
		...
		local charge = this.new("scripts/skills/actives/actives_gorge_skill");
	...
	function onHorseDamageReceived( _attacker, _skill, _hitInfo )
	{
		...
		else if (_attacker == null || !_attacker.isAlive())
		{
		}
		else if (_attacker.getTile().getDistanceTo(this.getContainer().getActor().getTile()) > 1)
		{
		}
		else if (_attacker.getFlags().has("lindwurm"))
		{
		}
		else if ((_attacker.getFlags().has("body_immune_to_acid") || _attacker.getArmor(this.Const.BodyPart.Body) <= 0) && (_attacker.getFlags().has("head_immune_to_acid") || _attacker.getArmor(this.Const.BodyPart.Head) <= 0))
		{
		}
		else if (poison == null)
		{
